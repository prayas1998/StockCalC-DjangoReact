...
import { type ChangeEvent, type KeyboardEvent, forwardRef, type ClipboardEvent, useState } from "react"
...
const NumericInput = forwardRef<HTMLInputElement, NumericInputProps>(
  ({ value, onChange, allowDecimal = true, min, max, maxDecimalPlaces, className, ...props }, ref) => {
    ...
    const handleKeyDown = (e: KeyboardEvent<HTMLInputElement>) => {
      ...
      const isKeyboardShortcut = (e.ctrlKey || e.metaKey) && ["c", "v", "a", "x", "z"].includes(e.key.toLowerCase())
      ...
      const minusAllowed =
        e.key === "-" && e.currentTarget.selectionStart === 0 && !value.includes("-") && (min === undefined || min < 0)
    ...
    return (
      <div className="relative">
        <Input
          ...
          onCopy={handleCopy}
          onBlur={handleBlur}
          className={cn("transition-all duration-200 focus:ring-2 focus:ring-offset-2", className)}
          {...props}
        />
        {showError && (
          <div className="absolute z-10 top-full mt-2 left-0 right-0 bg-red-50 dark:bg-red-900/50 text-red-700 dark:text-red-200 text-xs p-2 rounded-lg border border-red-200 dark:border-red-800 shadow-lg">
